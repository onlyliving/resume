    ...
    render() {
        ...
        const projectItems = projects.map((project) => {
            const links = project.linkList.map((link) => {
                return (
                    <li key={link.title}>
                        <a className="point-link" href={link.link} target="_blank" rel="noopener noreferrer">
                            {link.title}
                        </a>
                    </li>
                );
        ...
        return (
            <article className="content-box content-box--side-project">
                <h2 id="sideProjectHead" className="content-box__head">
                    <span>Side Project</span>
                </h2>
                <ul className="side-project-list">{projectItems}</ul>
            </article>
        );
    }
}
